...
class RealMLModels:
    ...
    def prepare_target(self, data):
        ...
        if 'disposition' in data.columns:
            ...
            planet_keywords = ['CONFIRMED', 'CANDIDATE', 'confirmed', 'candidate']
            data['target'] = data['disposition'].apply(
                lambda x: 1 if any(keyword in str(x) for keyword in planet_keywords) else 0
            )
            return data['target']
        return None
    ...
    def predict(self, data, model_name):
        ...
        probabilities = model.predict_proba(X_scaled)[:, 1]  # Probability of class 1 (planet)
...
def show_model_comparison(real_models=None):
    ...
    # Show real model performance if available
    if real_models and real_models.trained:
        st.subheader("🎯 Real Model Performance (Trained on Your Data)")
        
        col1, col2 = st.columns(2)
        
        with col1:
            st.write("**Random Forest Performance:**")
            rf_metrics = real_models.performance_metrics.get('Random Forest', {})
            if rf_metrics:
                st.metric("Accuracy", f"{rf_metrics.get('accuracy', 0):.3f}")
                st.metric("F1-Score", f"{rf_metrics.get('f1', 0):.3f}")
        
        with col2:
            st.write("**SVM Performance:**")
            svm_metrics = real_models.performance_metrics.get('SVM', {})
            if svm_metrics:
                st.metric("Accuracy", f"{svm_metrics.get('accuracy', 0):.3f}")
                st.metric("F1-Score", f"{svm_metrics.get('f1', 0):.3f}")
    ...
    recall = [float(x.strip('%'))/100 for x in comparison_data['Recall']]
    precision = [float(x.strip('%'))/100 for x in comparison_data['Precision']]
...
if not kepler_df.empty and not k2_df.empty and not tess_df.empty:
    ...
    if not master_df.empty:
        ...
        with col1:
            st.markdown("""
            **Train real machine learning models on your exoplanet data:**
            - Random Forest Classifier
            - Support Vector Machine (SVM)
            - Uses real disposition labels for training
            """)
        ...
        with col2:
            st.metric("Available Training Data", f"{len(master_df)} planets")
            if 'disposition' in master_df.columns:
                disposition_counts = master_df['disposition'].value_counts()
                st.write("**Disposition Distribution:**")
                for disp, count in disposition_counts.items():
                    st.write(f"- {disp}: {count}")
        ...
        *Upload new exoplanet data to test all AI models with automatic performance analysis!*
        ...
        - Uses REAL ML models when trained, simulated as fallback
        ...
        # Initialize model simulator with real models
        ...
        if test_data is not None:
            ...
            if st.button("🚀 Run All Model Tests", type="primary") or run_comprehensive:
                ...
                for model_name, results in all_predictions.items():
                    with st.container():
                        # Show if using real or simulated model
        ...
        show_model_comparison(real_ml_models)
