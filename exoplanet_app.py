...
class RealMLModels:
    """Real Machine Learning Models for Exoplanet Classification"""
    ...
    def train_models(self, master_df):
        """Train real ML models on the exoplanet data"""
        try:
            # Prepare the data
            if 'disposition' in master_df.columns:
                # Create binary target
                master_df = master_df.copy()
                master_df['target'] = master_df['disposition'].apply(
                    lambda x: 1 if str(x).upper() in ['CONFIRMED', 'CANDIDATE'] else 0
                )
                
                # Select features and target
                model_data = master_df[self.feature_columns + ['target']].dropna()
                
                if len(model_data) < 50:
                    st.warning("⚠ Not enough data for training real models")
                    return False
                ...
                self.rf_metrics = self._calculate_metrics(self.rf_model, X_test_scaled, y_test)
                self.svm_metrics = self._calculate_metrics(self.svm_model, X_test_scaled, y_test)
    ...
    def predict(self, data):
        """Make predictions with real models"""
        ...
        try:
            ...
            # Scale features
            X_pred_scaled = self.scaler.transform(X_pred[self.feature_columns])
            
            # Make predictions
            rf_predictions = self.rf_model.predict(X_pred_scaled)
            rf_probabilities = self.rf_model.predict_proba(X_pred_scaled)
            
            svm_predictions = self.svm_model.predict(X_pred_scaled)
            svm_probabilities = self.svm_model.predict_proba(X_pred_scaled)
            
            return {
                'Random Forest': {
                    'predictions': rf_predictions,
                    'confidences': rf_probabilities[:, 1],
                    'metrics': self.rf_metrics
                },
                'SVM': {
                    'predictions': svm_predictions,
                    'confidences': svm_probabilities[:, 1],
...
def plot_confusion_matrices(metrics_dict):
    ...
    for idx, (model_name, metrics) in enumerate(metrics_dict.items()):
        if metrics and 'confusion_matrix' in metrics:
            if isinstance(metrics['confusion_matrix'], dict):
                # For simulated models
                cm = metrics['confusion_matrix']
                matrix = np.array([[cm['tp'], cm['fp']], [cm['fn'], cm['tn']]])
            else:
                # For real models (sklearn format)
...
if not kepler_df.empty and not k2_df.empty and not tess_df.empty:
    ...
    if not master_df.empty:
        ...
        st.header("🤖 Real Machine Learning Models")
        
        real_models = RealMLModels()
        
        with st.spinner("Training real ML models on exoplanet data... 🧠"):
            training_success = real_models.train_models(master_df)
        ...
        if training_success:
            ...
            with col1:
                ...
                if real_models.rf_metrics:
                    st.metric("Accuracy", f"{real_models.rf_metrics['accuracy']:.3f}")
                    st.metric("Precision", f"{real_models.rf_metrics['precision']:.3f}")
                    st.metric("Recall", f"{real_models.rf_metrics['recall']:.3f}")
                    st.metric("F1-Score", f"{real_models.rf_metrics['f1']:.3f}")
            
            with col2:
                st.subheader("SVM Performance")
                if real_models.svm_metrics:
                    st.metric("Accuracy", f"{real_models.svm_metrics['accuracy']:.3f}")
                    st.metric("Precision", f"{real_models.svm_metrics['precision']:.3f}")
                    st.metric("Recall", f"{real_models.svm_metrics['recall']:.3f}")
                    st.metric("F1-Score", f"{real_models.svm_metrics['f1']:.3f}")
        else:
            st.warning("⚠ Could not train real models - using simulated versions")
        ...
        *Upload new exoplanet data to test all AI models with automatic performance analysis!*
        ...
        🚀 *Features:*
        - Tests both real ML models and simulated models
        - Automatic performance comparison
        - Robust error handling for any data format
        - Professional metrics with detailed analysis
        """)
        ...
        with col2:
            ...
            run_comprehensive = st.checkbox("🏃‍♂ Run comprehensive analysis", value=True)
        ...
        if test_data is not None:
            ...
            if st.button("🚀 Run All Model Tests", type="primary") or run_comprehensive:
                ...
                # Test real models first
                if training_success:
                    real_results = real_models.predict(processed_data)
                    if real_results:
                        all_predictions.update(real_results)
                        for model_name, results in real_results.items():
                            if results['metrics']:
                                all_metrics[model_name] = results['metrics']
                
                # Test simulated models (Quantum and fallback for others)
                models_to_simulate = ['Quantum-Hybrid']
                if not training_success:
                    models_to_simulate.extend(['Random Forest', 'SVM'])
                
                for model_name in models_to_simulate:
                    if model_name == 'Random Forest':
                        predictions, confidences = model_simulator.predict_random_forest(processed_data)
                    elif model_name == 'SVM':
                        predictions, confidences = model_simulator.predict_svm(processed_data)
                    else:  # Quantum-Hybrid
                        predictions, confidences = model_simulator.predict_quantum_hybrid(processed_data)
                ...
                for model_name, results in all_predictions.items():
                    with st.container():
                        ...
                        <div class="model-card {'quantum-card' if 'Quantum' in model_name else 'svm-card' if 'SVM' in model_name else ''}">
                            <h3>🧠 {model_name} {'(Real)' if model_name in ['Random Forest', 'SVM'] and training_success else '(Simulated)'}</h3>
                        </div>
                        """, unsafe_allow_html=True)
                ...
                with st.expander("📋 Show Detailed Prediction Results"):
                    ...
                    # Add agreement column if multiple models
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["Planet Sizes", "Orbital Periods", "Temperatures", "Data Quality"])
