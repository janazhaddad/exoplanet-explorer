...
class ExoplanetQAAssistant:
    def __init__(self, data):
        self.data = data
        self.planet_keywords = ['planet', 'exoplanet', 'candidate', 'confirmed', 'false positive']
        self.mission_keywords = ['kepler', 'k2', 'tess', 'mission', 'telescope']
        self.feature_keywords = ['radius', 'size', 'temperature', 'period', 'orbit', 'gravity', 'teff', 'logg']
        self.model_keywords = ['model', 'prediction', 'accuracy', 'performance', 'random forest', 'svm', 'quantum']
        self.stat_keywords = ['how many', 'count', 'average', 'mean', 'largest', 'smallest', 'hottest', 'coldest']
    ...
    def analyze_data(self):
        ...
        stats['missions'] = self.data['mission'].value_counts().to_dict() if 'mission' in self.data.columns else {}
        ...
        # Feature statistics
        numeric_features = ['pl_orbper', 'pl_rade', 'st_teff', 'st_rad', 'st_logg', 'pl_eqt']
    ...
    def answer_question(self, question):
        ...
        if any(keyword in question_lower for keyword in self.mission_keywords):
            if 'how many missions' in question_lower:
                return f"There are {len(stats['missions'])} different space missions in the dataset: {', '.join(stats['missions'].keys())}."
            
            elif 'kepler' in question_lower:
                kepler_count = stats['missions'].get('Kepler', 0)
                return f"The Kepler mission has {kepler_count} observed objects in the dataset."
            
            elif 'k2' in question_lower:
                k2_count = stats['missions'].get('K2', 0)
                return f"The K2 mission has {k2_count} observed objects in the dataset."
            
            elif 'tess' in question_lower:
                tess_count = stats['missions'].get('TESS', 0)
                return f"The TESS mission has {tess_count} observed objects in the dataset."
        ...
        # Planet count questions
            ...
            if 'confirmed' in question_lower:
                ...
                return f"There are {confirmed} confirmed exoplanets in the dataset."
            elif 'candidate' in question_lower:
                candidate = stats['dispositions'].get('CANDIDATE', 0)
                return f"There are {candidate} candidate exoplanets in the dataset."
            else:
                total_planets = stats['dispositions'].get('CONFIRMED', 0) + stats['dispositions'].get('CANDIDATE', 0)
                return f"There are {total_planets} total potential planets (confirmed + candidates) in the dataset."
        
        # Feature statistics questions
        elif 'largest planet' in question_lower or 'biggest planet' in question_lower:
            if 'pl_rade_max' in stats:
                return f"The largest planet in the dataset has a radius of {stats['pl_rade_max']:.2f} Earth radii."
        
        elif 'smallest planet' in question_lower:
            if 'pl_rade_min' in stats:
                return f"The smallest planet in the dataset has a radius of {stats['pl_rade_min']:.2f} Earth radii."
        
        elif 'hottest planet' in question_lower:
            if 'pl_eqt_max' in stats:
                return f"The hottest planet in the dataset has an equilibrium temperature of {stats['pl_eqt_max']:.1f} Kelvin."
        
        elif 'longest orbital period' in question_lower:
            if 'pl_orbper_max' in stats:
                return f"The planet with the longest orbital period orbits its star every {stats['pl_orbper_max']:.1f} days."
        
        elif 'average' in question_lower:
            if 'radius' in question_lower and 'pl_rade_mean' in stats:
                return f"The average planet radius in the dataset is {stats['pl_rade_mean']:.2f} Earth radii."
            
            elif 'temperature' in question_lower and 'pl_eqt_mean' in stats:
                return f"The average equilibrium temperature of planets in the dataset is {stats['pl_eqt_mean']:.1f} Kelvin."
            
            elif 'orbital period' in question_lower and 'pl_orbper_mean' in stats:
                return f"The average orbital period of planets in the dataset is {stats['pl_orbper_mean']:.1f} days."
        
        # Model performance questions
        elif any(keyword in question_lower for keyword in self.model_keywords):
            if 'best model' in question_lower or 'which model' in question_lower:
                return "Based on our analysis, Random Forest typically provides the best balance of accuracy and interpretability for exoplanet classification."
            
            elif 'accuracy' in question_lower:
                return "Our models achieve 85-95% accuracy in classifying exoplanets, with Random Forest performing most consistently across different datasets."
        
        # Data quality questions
        elif 'data quality' in question_lower or 'missing data' in question_lower:
            missing_percentage = (self.data.isnull().sum().sum() / (self.data.shape[0] * self.data.shape[1])) * 100
            return f"The dataset has approximately {missing_percentage:.1f}% missing values overall, which we handle through our Data Doctor feature."
        
        # General dataset info
        elif 'total' in question_lower and 'object' in question_lower:
            return f"The dataset contains {stats['total_objects']} total celestial objects from multiple space missions."
        
        # Default response for unrecognized questions
        else:
            return "I can answer questions about:\n• Planet counts and statistics\n• Mission data (Kepler, K2, TESS)\n• Planet characteristics (size, temperature, orbits)\n• Model performance\n• Data quality\n\nTry asking about specific missions, planet sizes, or dataset statistics!"
    ...
    if not master_df.empty:
        ...
        with quick_col1:
            if st.button("Kepler Stats", use_container_width=True):
                st.session_state.quick_question = "How many objects did Kepler find and how many are confirmed planets?"

        with quick_col2:
            if st.button("Largest Planet", use_container_width=True):
                st.session_state.quick_question = "What's the largest planet across all missions?"
        ...
        # --- ENHANCED PLANET CELEBRATION SECTION ---
        st.markdown("---")
        ...
        st.markdown(
            ...
            .rocket1 { animation-delay: 0.2s; }
            .rocket2 { animation-delay: 0.7s; }
            .rocket3 { animation-delay: 1.2s; }
            ...
            <div class="mission-stats">
                <h3>🌌 Mission Discoveries</h3>
                <p>Exploring the universe through Kepler, K2, and TESS telescopes</p>
            </div>
            
            <div class="floating-planets">
                <span class="planet1">🪐</span>
                <span class="rocket1">🚀</span>
                <span class="planet2">🌍</span>
                <span class="rocket2">🛰</span>
                <span class="planet3">🌕</span>
                <span class="rocket3">👨‍🚀</span>
